...
var path = require('path'),
    ...
    processDocs = require('./process-docs'),
    processBlocks = require('./process-blocks'),
    fsHelpers = require('./fs-helpers'),

    bundleName = 'index',
    pathToBundle = path.resolve(__dirname, '..', 'bundles', bundleName),
    ...
    return del(outputFolder, { force: true }).then(function() {
        return Promise.all(data.langs.reduce(function(promises, lang) {
            ...
            return promises.concat(
                ...
                cpy([
                    path.join(__dirname, '..', 'bundles', 'index', 'index.{css,js}'),
